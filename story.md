...
- 休息（`rest`）是“更慢的恢复”：单次恢复降低、耗时更长；不能靠频繁休息抵消战斗压力。
- 经济不再鼓励无脑刷钱：矿石/草药售卖收益下调，更偏向补贴而非主要收入来源。
...
- 敌人进攻性整体上调：多数敌人攻击力提高，战斗更强调护身、打断蓄力、与技能节奏。
- 分支首领与章末战更需要“少量准备”：携带 1-2 个护身符与少量药剂即可；不鼓励堆叠大量消耗品。
- 浓雾侵蚀（`fog`）：部分敌人会附带“浓雾持续伤害”；`fog_mask` 可免疫，`fogward_talisman` 可在持续期间隔绝。
...
- `evasion`：会以一定概率闪避玩家的物理攻击（普攻/部分近战技能）。不再“跳过敌方本回合攻击”。
- `focus` / `focus_tea`：在持续期间提供“必中”效果，用于稳定命中高闪避目标。
- `curses`：部分敌人在战斗中会施加 `cursed`（提高后续受伤），可用 `purify` 清除。
- `summon`：部分敌人会生成“堆叠压力”（提升敌方输出），可用 `sweep` / `explosive_trap` / `bound_charm` 清掉堆叠。
